...
class User():
    ...
    def password(self, pwd):
        ...
        - Hash `pwd` using a secure algorithm (e.g., bcrypt) before assigning to `__password`
        """
        if pwd is None or not isinstance(pwd, str):
            self.__password = None
        else:
            # Use a secure password hashing algorithm instead of MD5
    ...
    def is_valid_password(self, pwd):
        ...
        - Compare `__password` and the hash value of `pwd`
        """
        if pwd is None or not isinstance(pwd, str):
            return False
